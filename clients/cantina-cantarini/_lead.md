# cantina-cantarini

- **Found:** 2026-08-28
- **Address:** Piazza Sallustio, 12, 00187 Roma
- **Phone:** 06 4743341 (also listed: 06 485528)
- **Current website:** ristorantecantinacantarini.it, exists and loads, but it's a 2014-era site (CMS "paginesi.it", jQuery 1.10, no real mobile layout, generic bxSlider/flexSlider setup). Not dead, just badly dated: a real opportunity, not a "no site" lead.
- **Instagram:** https://www.instagram.com/cantinacantarini/ (confirmed)
- **Facebook:** https://www.facebook.com/cantina.cantarini/ (confirmed)
- **Email:** santinafattori1964@gmail.com, found printed in the footer of their own site, not invented.
- **Sources:** their own site (address, phone, hours, email, real logo, real food photos), Touring Club Italiano and Turismo Roma (history: founded 1903 by Mariano Cantarini), camillabaresani.com (2019 feature article with a real exterior photo of the storefront in Piazza Sallustio).
- **Status:** demo-built
- **Demo URL:** (not yet deployed this round)
- **Sold:** no
- **Notes:**
  - Real branding used throughout, no stock. Logo: cropped directly from their own site's logo image (the engraved-fish medallion, left portion of a 400x120 wordmark) — a clean crop, not a redesign, since the icon itself is already roughly square.
  - Brand colors pulled from their site's own CSS (`brown.css` theme): `#a52a2a` primary, `#800000` accent, matching the red of the fish-medallion logo.
  - Photos: `hero.jpg` is a real exterior storefront shot from a 2019 press feature (camillabaresani.com). `about.jpg` (ravioli) and `gallery-1.jpg`/`gallery-2.jpg` (carbonara, fritto misto) are all real photos pulled directly from their own site — `about.jpg` even has "Cant Cant" printed on the plate rim, confirming it's genuinely their dish, not stock. These are low-res (300x188, from 2014) but authentic.
  - Their site also had 2 higher-res "food" photos added in 2023 (a stew and a chicken parmigiana). Checked and rejected: EXIF `DateTimeOriginal` is identical to the upload timestamp (down to the minute) for both, studio-quality generic lighting, and one dish (parmigiana) isn't even on their actual Roman/Marchigian menu. Almost certainly stock the business itself added to pad the site out, not photos of their own food — did not reuse them.
  - No price list published anywhere online (their site has no prices, just dish names) — left blank in `config.js`, same as other trattoria clients.

## Outreach — not yet drafted or sent

This business already has a working, if dated, website with a confirmed real email — a genuinely warm, low-pressure pitch fits well here. Draft not written yet this round; will draft in normal prose and show it before sending anything, per standing process. No message has been sent on any channel for this lead yet.
