...
- **Notes:**
  ...
  - **Photos are stock** (Free Unsplash License, no attribution required — hero `photo-1671453359294-573a40fa1921`, gallery `photo-1755594461640-b800c6bafdfa` / `photo-1541401154946-62f8d84bd284` / `photo-1756831567129-a7f9e6ecb436`), **except `about.jpg`, updated 2026-08-26 to a real photo.** Instagram itself is unreachable by automated fetch (confirmed again this session — client-rendered SPA, no static photo URLs at all, not just blocked) — the real handle was never even recorded here (only the logo, sourced by Nikolai from a screenshot). Found one real, verifiable photo elsewhere instead: a cacio e pepe / gricia dish from RomaToday's own article about this trattoria, watermark cropped off. No other verifiable real photos found — swap the rest for real photos if/when the client buys, or if Nikolai can share an Instagram screenshot per the usual workaround.
