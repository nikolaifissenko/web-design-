...
Every client also gets a `theme.mood` in `config.js`, which activates a
`[data-mood="..."]` block in the shared `template/css/moods.css` (loaded
after `style.css`, on top of it). Colors and fonts still come from
`theme.primary/accent/font/headingFont`; moods only touch layout, shape,
and decoration, so any mood works with any client's own palette. Current
moods:

- **editorial**: asymmetric, magazine-style. Heritage artisans with a
  strong founder story (tailors, bookbinders, restorers).
- **rustic**: warm, tactile, slightly imperfect (tilted photos, a wavy
  section divider). Trattorie and family food businesses.
- **bold**: full-color-blocked sections, hard edges, grayscale-to-color
  photo hover. Modern/industrial services.
- **boutique**: spacious, restrained, ghost-outline buttons. B&Bs and
  upscale services.
- **vintage**: warm rounded display serif (Yeseva One), a market-placard
  price list (dashed rule, rounded price-tag badge), photos framed like an
  old family print (white border, sepia that clears to color on hover).
  1920s to 1960s Italian shop-packaging character. Genuinely old,
  non-imperial businesses.

**Picking a mood is a cultural-fit judgment, not just "this business is
old."** An "imperial" mood existed for one day: Trajan-style capitals,
Roman numerals, marble-plaque photo framing, built for two Ghetto kosher
bakeries because they're old Roman institutions. It was wrong and got
scrapped, because ancient-Rome/SPQR iconography is specifically imperial
and pagan Roman, and these are specifically Jewish-Roman institutions
with their own five-century heritage, not generic "old Rome." The fix
was "vintage" above: warm and dignified without borrowing a different
culture's visual language. Before assigning or designing a mood, ask what
this business's history actually is, not just how many decades it has
been around. When unsure, that's worth surfacing to Nikolai rather than
guessing at the most obvious trope.
...
4. Re-diff: for every difference found in step 2, confirm the same
   ...
   `Edit`, not a full revert; the goal is the new template features *and*
...
roughly 1 to 2 minutes; poll for it, don't assume).
...
- Root `CLAUDE.md`: the underlying principle (real branding over
  ...
  and the outreach writing/sending rules.
